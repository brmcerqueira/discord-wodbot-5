...
const documents: {
    [id: string]: { document: pdf.PDFDocument, file: string}
} = {};

export async function load(): Promise<void> {
    for await (const path of Deno.readDir("./characters")) {
        if (path.isFile && path.name.endsWith(".pdf")) {
            const arrayBuffer = await Deno.readFile(`./characters/${path.name}`);
            ...
            documents[id] = {document, file: path.name};

            const userIdStart = path.name.lastIndexOf('[');
            const userIdEnd = path.name.lastIndexOf(']');

            if (userIdStart > -1 && userIdEnd > -1 && userIdStart < userIdEnd) {
                const userId = path.name.substring(userIdStart + 1, userIdEnd);
                users[userId] = id;
                await buildLightPdf(userId, path.name);
            }      
        }
    }
}
...
async function updatePdf(id: string, update: (form: pdf.PDFForm) => void) {
    ...
    await Deno.writeFile(`./characters/${pair.file}`,
